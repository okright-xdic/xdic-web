...
export default function EnglishGuideHubPage() {
  ...
  const guideIconByLabel: Record<string, string> = {
    '이동 동사': '🧭',
    '지각 동사': '👁️',
    '말하기 동사': '💬',
    '행위 동사': '🛠️',
    ...
    <div className="min-h-screen bg-white text-slate-900">
      <main className="w-full max-w-5xl mx-auto px-3.5 sm:px-5 py-5 md:py-7">
        ...
        <section className="mt-5">
          ...
          <h2 className="mt-0.5 text-[17px] md:text-[20px] font-black text-slate-900">
            해설 주제 한눈에 보기
          </h2>
          ...
          <div className="mt-2.5 grid grid-cols-1 sm:grid-cols-3 gap-2">
            <a
              href="#word-choice"
              className="group rounded-xl border border-blue-100 bg-blue-50/30 px-3 py-2.5 hover:border-blue-200 hover:bg-blue-50/60 transition-all"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-[13px] md:text-[14px] font-extrabold text-slate-900">
                      기본 동사와 표현 선택
                    </h3>
                    <p className="mt-0.5 text-[10px] md:text-[11px] text-slate-500">
                      go·come · see·look·watch
                    </p>
                  </div>
                </div>
            ...
            <a
              href="#grammar"
              className="group rounded-xl border border-violet-100 bg-violet-50/30 px-3 py-2.5 hover:border-violet-200 hover:bg-violet-50/60 transition-all"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="text-[23px] leading-none" aria-hidden="true">
            ...
            <a
              href="#polysemy"
              className="group rounded-xl border border-emerald-100 bg-emerald-50/30 px-3 py-2.5 hover:border-emerald-200 hover:bg-emerald-50/60 transition-all"
            >
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="text-[23px] leading-none" aria-hidden="true">
        ...
        {guideGroups.map((group) => (
          ...
          >
            <div className="border-b border-slate-200 pb-2.5">
              <h2 className="text-[17px] md:text-[20px] font-extrabold text-slate-900">
                {group.title}
              </h2>

              <p className="mt-1 text-[11px] md:text-[12px] leading-5 text-slate-500 break-keep">
                {group.description}
              </p>
            </div>
            ...
            <div className="mt-3 grid gap-2.5 md:grid-cols-2">
              {group.guides.map((guide) => (
                <article
                  key={guide.href}
                  className="rounded-xl border border-slate-200 bg-white px-3.5 py-3.5 shadow-sm"
                >
                  <div className="flex items-start gap-2.5">
                    <span
                      className="text-[22px] leading-none mt-0.5 shrink-0"
                      aria-hidden="true"
                    >
                      {guideIconByLabel[guide.label] || '📘'}
                    </span>

                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] md:text-[11px] font-extrabold text-blue-600">
                        {guide.label}
                      </p>

                      <h3 className="mt-0.5 text-[15px] md:text-[17px] font-extrabold text-slate-900 leading-tight">
                        {guide.title}
                      </h3>

                      <p className="mt-0.5 text-[11px] md:text-[12px] font-bold text-slate-700">
                        {guide.subtitle}
                      </p>
                    </div>
                  </div>

                  <p className="mt-2 text-[11px] md:text-[12px] leading-5 text-slate-600">
                    {guide.summary}
                  </p>

                  <div className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-[11px] md:text-[12px] font-semibold text-slate-700">
                    {guide.examples}
                  </div>

                  <Link
                    href={guide.href}
                    className="inline-flex mt-2.5 text-[11px] md:text-[12px] font-extrabold text-blue-700 hover:text-blue-900"
                  >
                    해설 읽기 →
                  </Link>
                </article>
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[17px] md:text-[19px] font-extrabold">
              X-DIC 번역가 해설의 편집 원칙
            </h2>
          </div>
          ...
          <p className="mt-1.5 text-[11px] md:text-[12px] leading-5 text-slate-600">
            ...
            X-DIC 해설에서는 다음 판단 과정을 반복해서 적용합니다.
          </p>
          ...
          <div className="mt-3 grid gap-2.5 md:grid-cols-2">
            {editorialPrinciples.map((item) => (
              ...
              >
                <h3 className="text-[12px] md:text-[13px] font-extrabold text-slate-900">
                  {item.title}
                </h3>
